...
class Preprocessor:
    ...
    def sentence_to_indices(self, sentence):
        ...
        for i in words:
            indices.append(self.embedding.to_index(i))
        return indices

    #n_workers mean how many process to do at a time.
    def collect_words(self, data_path, n_workers=4):
        ...
        #to make sure the words are not repeated.
        utterances = list(set(utterances))
        #//means divide and only get the integer part.
        chunks = [
    ...
    def get_dataset(self, data_path, n_workers=4, dataset_args={}):
        ...
        #padding is an indices list for every </s>
    ...
    def preprocess_sample(self, data):
        ...
        for message in data['messages-so-far']:
            if flag == 1:
                processed['context'].append(self.sentence_to_indices('speakerone'))
                flag = 2
            else:
                processed['context'].append(self.sentence_to_indices('speakertwo'))
                flag = 1
        ...
        #in this way, the correct options will always be in the front of the negative options.
        return processed
