...
class BasePredictor():
	def __init__(self,
		...
		if device is not None:
			...
			self.device = torch.device('cuda:0' if torch.cuda.is_available() #我把CUDA改成1囉
									   else 'cpu')
	...
	def _run_epoch(self, dataloader, training):
		...
		if training:
			iter_in_epoch = min(len(dataloader), self.max_iters_in_epoch) # data/batch_size
		...
		for i, batch in trange:
			...
			if training:
				...
				if i % self.grad_accumulate_steps == 0:
					self.optimizer.zero_grad() # TODO: call zero gradient here

				batch_loss.backward()
				
				# accumulate gradient - step
				if (i + 1) % self.grad_accumulate_steps == 0:
					self.optimizer.step() # TODO: update gradient here
